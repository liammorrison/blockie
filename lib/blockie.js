...
function resolveAllInstances() {
    //Updates the allInstances array to contain all different instance types' arrays.
    updateAllInstances();

    //Sorts through each sub-array in allInstances. 
    for (var i = 0; i < allInstances.length; i++) {
        //The for loop's length is determined before it starts to avoid missing the first element.
        var initialArrayLength = allInstances[i].length;
        var subArray = allInstances[i];

        //Rejects each instance's Promises and timeouts and destroys the instances.
        for (var j = initialArrayLength - 1; j >= 0; j--) {
            var instance = subArray[j];
            instance.externalResolve();
            clearTimeout(instance.timeout);
            subArray.splice(j, 1);
        };
