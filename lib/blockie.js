...
//Bypasses Google's anti-autoplay system by starting to play songs only once the tab is interacted with.
document.addEventListener("mousedown", initializeSound);

function initializeSound() {
    stockSong.play();
    stockSong.loop = true;
    document.removeEventListener("mouseover", initializeSound);
};
...
function initializeLevelMenu() {
    ...
    function checkFollowMouse() {
        followMouse = true;
        var stopFollowingMouse = setTimeout(function () {
            followMouse = false;
        }, 1000);
    };
