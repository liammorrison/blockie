        ...
        return regeneratorRuntime.wrap(function _callee$(_context) {
            while (1) {
                switch (_context.prev = _context.next) {
                    ...
                    case 4:
                        _context.next = 6;
                        return Promise.all([createActivePoint(5 * 16, center - 8, 0, 6), fireBomb(2, 3 * 16 + 2, maxEdge - 4, 10 * 16 - 4, 0, 6), fireBomb(2, 19 * 16 + 2, maxEdge - 4, 10 * 16 - 4, 0, 6), fireVerticalLaser(15 * 16 + 8, 16, 0, 6)]);

                    case 6:
                        _context.next = 8;
                        return Promise.all([createActivePoint(26 * 16, center - 8, 0, 7), fireBomb(2, 3 * 16 + 2, maxEdge - 4, 10 * 16 - 4, 0, 7), fireBomb(2, 19 * 16 + 2, maxEdge - 4, 10 * 16 - 4, 0, 7), fireMovingVerticalLaser(20 * 16, 16, -1, 0, 7)]);

                    case 8:
                        _context.next = 10;
                        return Promise.all([createPassivePoint(3 * 16 + 10, center - 8, 0, 11), createPassivePoint(center - 8, 3 * 16 + 10, 0, 15), createActivePoint(center - 8, 28 * 16 - 10, 0, 19), fireBomb(3 * 16 + 2, 3 * 16 + 2, 10 * 16 - 4, 10 * 16 - 4, 0, 19), fireBomb(3 * 16 + 2, 19 * 16 + 2, 10 * 16 - 4, 10 * 16 - 4, 0, 19), fireBomb(19 * 16 + 2, 3 * 16 + 2, 10 * 16 - 4, 10 * 16 - 4, 0, 19), fireBomb(19 * 16 + 2, 19 * 16 + 2, 10 * 16 - 4, 10 * 16 - 4, 0, 19), fireMovingVerticalLaser(0, 16, 1.5, 0, 7), fireMovingVerticalLaser(0, 16, 1.5, 2, 7), fireMovingHorizontalLaser(0, 16, 1.5, 4, 7), fireMovingHorizontalLaser(0, 16, 1.5, 6, 7), fireMovingHorizontalLaser(maxEdge - 16, 16, -1.5, 8, 7), fireMovingHorizontalLaser(maxEdge - 16, 16, -1.5, 10, 7)]);
...
function gameLoop() {
    ...
    if (!recoveringFromDash) {
        ...
        if (keysDown[16] && allowDashAgain && (xInput !== 0 || yInput !== 0)) {
            ...
            var _endDashRecoveryTime = 0.3;
            var _endDashRecovery = setTimeout(function () {
                recoveringFromDash = false;
                blockie.state = "playing";
                removeCurrentTimer(_endDashRecovery);
            }, _endDashRecoveryTime * 1000);
            addCurrentTimer(_endDashRecovery);

            var _resetAllowDashAgainTime = 0.9;
            var _resetAllowDashAgain = setTimeout(function () {
                allowDashAgain = true;
                removeCurrentTimer(_resetAllowDashAgain);
            }, _resetAllowDashAgainTime * 1000);
            addCurrentTimer(_resetAllowDashAgain);
        } else {
            blockie.speed = 2;
        }
